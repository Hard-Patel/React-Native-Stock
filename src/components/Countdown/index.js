...
const DEFAULT_DIGIT_STYLE = {
  backgroundColor: applicationTheme.colors.message[700],
};
const DEFAULT_DIGIT_TXT_STYLE = {
  color: applicationTheme.colors.absoluteWhite[900],
};
...
class CountDown extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    } else if (this.props.rounds !== prevProps.rounds) {
      this.setState({
        lastUntil: prevState.until,
        until: Math.max(prevProps.until, 0),
      });
    ...
    return (
      <View style={digitStyle}>
        <Text
          style={[
            styles.digitTxt,
            { fontSize: size, fontFamily: fontFamily.regular, fontSize: 12 },
            digitTxtStyle,
          ]}
        >
          {d}
  ...
  renderSeparator = () => {
    ...
    return (
      <View style={{ justifyContent: "center", alignItems: "center" }}>
        <Text style={[styles.separatorTxt, { fontSize: 12 }, separatorStyle]}>
          {":"}
        </Text>
      </View>
    );
